...
public class City extends Geometry {
	...
	/** 2D remote coordinates belonging to this city */
	...
	public City(final String name, final int x, final int y, final int rx, final int ry, 
			final int radius, final String color) {
		this.name = name;
		pt = new Point2D.Float(x, y);
		remotePt = new Point2D.Float(rx, ry);
		this.radius = radius;
		this.color = color;
	}
	...
	/**
	 * Returns a Point2D instance representing the Metropole location that the
	 *  city is within.
	 */
	public Point2D getRemotePt() {
