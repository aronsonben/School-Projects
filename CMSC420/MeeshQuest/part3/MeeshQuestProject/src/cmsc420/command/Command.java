...
public class Command {
	...
	/** store mapping of metropole points to corresponding PMQuadtrees */
	//protected HashMap<Metropole, PMQuadtree> metropoles = new HashMap<Metropole, PMQuadtree>();
	protected HashMap<Point2D, PMQuadtree> metropoles = new HashMap<Point2D, PMQuadtree>();
	
	/** number of metropoles in spatial data structure */
	protected int metropoleNum = 0;
	
	/** stores metropoles in a spatial data structure */
	...
	/** spatial width of the PR Quadtrees, located within PM Quadtree */
	protected int localSpatialWidth;

	/** spatial height of the PR Quadtrees, located within PM Quadtree */
	...
	public void processCommands(final Element node) {
		...
		// Should be deleted later once PR & PM structure is set up (PMs created upon city creation in a new metropole)
		if (pmOrder == 1) {
	...
	public void processCreateCity(final Element node) {
		...
		if (citiesByLocation.contains(city)) {
			...
			/* metropole checking / creation */
			final Metropole metropole = new Metropole(String.valueOf(metropoleNum++), remoteX, remoteY, color);
			final Point2D metropole1 = new Point(remoteX, remoteY);
			
			if(!metropoles.containsKey(metropole1)) {
				...
				metropoles.put(metropole1, pmQuad);
			} else {
				
			}
			// End metropole processing //
	...
	public void processMapRoad(Element node) {
		...
		if (!citiesByName.containsKey(start)) {
			...
			try {
				...
				if (Inclusive2DIntersectionVerifier.intersects(citiesByName.get(start).toPoint2D(), 
						new Rectangle2D.Float(0, 0, remoteSpatialWidth, remoteSpatialHeight))
